...
经典 LIO（如 FAST-LIO2 一类管线）普遍隐含**静态世界假设**：点云配准把场景当作刚体背景，动态物体上的点会被误当作稳定几何约束，从而在**动态占比高**或**静态几何稀疏/退化**时引入位姿漂移甚至发散。常见做法是「先分割/剔除动态，再做里程计」，但这会陷入一种**鸡生蛋式的循环依赖**：要准确定位需要可靠的静态点，而要可靠地区分动静态又往往依赖已经准确的位姿。
...
若只记一句话：**把点在 $(x,y,z,t)$ 里的局部几何与运动线索压进一个四维法向里，并让它参与每次 ICP 的数据关联，从而在动态、几何退化场景里仍尽量用「真静态」约束去拉住位姿；建图端再用空间一致性收拾假阳性。**
