...
### 自动验证
- 编译命令：`catkin_make --pkg costmap_2d`；确保零报错

### 手动验证
- **DenoiseLayer**：在有传感器噪声的场景中加入该层，观察 costmap 是否消除了孤立噪点
- **RangeSensorLayer**：开启超声波或 TOF 传感器，确认障碍物正确被写入 costmap
- **KeepoutFilter**：发布一张含禁区的 OccupancyGrid，观察该区域在 costmap 中是否显示为致命障碍
