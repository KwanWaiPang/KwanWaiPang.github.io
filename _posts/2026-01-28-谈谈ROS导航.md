...
### 1. `src/costmap_2d_ros.cpp` 
ROS 节点的顶层接口。主要逻辑，图层管理

* `Costmap2DROS`类：主类，它封装了 `Costmap2D` 并与 ROS 系统集成。它管理图层的加载、更新、发布可视化信息，并通过 `tf` 将地图关联到机器人坐标系。
* `Costmap2D` 类： 核心数据结构，存储底层二维网格代价地图。
* 地图更新主线程`mapUpdateLoop`
...
* 体素层
    * 障碍层的 3D 版本，用于检测不同高度的障碍物（如桌子、悬挂物等）。
  * 
### 4. `src/costmap_2d.cpp`
