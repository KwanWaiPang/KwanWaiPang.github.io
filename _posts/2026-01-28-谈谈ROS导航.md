...
* 地图更新主线程`mapUpdateLoop`，根据设定的频率循环调用地图更新逻辑。
...
- **footprint管理**: 订阅并处理机器人模型（footprint/机器人在投影到地面的 2D 多边形轮廓），用于后续的膨胀和碰撞检查。
...
LayeredCostmap是实际存储数据的地方，Costmap2DROS只是它的一个“ROS 包装壳”。
