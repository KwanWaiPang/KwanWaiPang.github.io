...
这就是为什么前面会说「Agent 是由所有不需要智能的地方构成的部分」。
一个流程当中，所有能用固定的程序来解决、而不需要问大模型的地方，就是 Agent 发挥作用的地方。其实就是把模糊的分流逻辑交给大模型，根据语义识别出用户想做 A 还是 B；把确定的分流逻辑交给程序，比如 PDF 提取文本。最终的目标都是节省人类的时间、降低人的使用门槛罢了。
