...
author: kwanwaipang
toc: true
excerpt: "记录了AI生成译文的过程及《前汉演义》目录" # 【指定摘要内容】
---
...
```bash
对于博客"File/前汉演义/前汉演义84.md"中的译文，请你全部重新翻译，并且按照“以下为与上文《原文》逐段对应的现代汉语白话全译；括号内偶附演义夹批。”来做，注意，括号内附的演义夹批也需要一字一句翻译，谢谢。
请注意，是一字一句翻译为现代汉语，越详细越好。
# 译文放于各段原文之后的 NOTE。

你对于我的这个需求还有没有什么不明白的，有的话你就提出来。如果没有的话，你就可以开始执行了

#如有追问：
# 1. 要分节小标题
# 2. 是指把情节、对话、夹批都充分展开成白话、不压缩略写，尽量做到接近字字对译（几乎每个文言词都翻译）
```
...
* PS：原文从维基文库拉取，字体要转简体字；译文要求：段落体、逐句核对、现代白话、不漏译。先读取原文与现译文后直接改。
...
写入路径：`File/前汉演义/前汉演义XX.md`（`XX` 为两位回次）。**再次运行会整文件覆盖**，若已手写译文请先备份或改脚本逻辑。
...
脚本生成的 `# 译文` 初值为 `（译文生成中）`。实际生产时在 Cursor 用 Auto / Agent，按回次分批撰写白话述要（对齐 `File/前汉演义/前汉演义68.md`：开篇说明、若干 `##`、诗联要点、`## 回末史论（白话）`），再写回对应 md。**与抓取脚本解耦**，避免一次对话超长。
...
<div align="center">
  <img src="https://r-c-group.github.io/blog_media/images/ScreenShot_2026-04-18_225715_188.png" width="100%" />
