...
* 当然也可以单独push `git push git@github.com:yourName/repo1.git 分支名:分支名`
...
所有标记为 `both modified` 的文件都需要解决冲突。

手动编辑，保留你想要的内容，删除 `<<<<<<<`、`=======`、`>>>>>>>` 这三行标记。

或者在 VS Code 中点击冲突处上方出现的选项：
...
标记冲突已解决：
```bash
git add .gitignore   # 将解决后的文件标记为已解决
# 如果有多个冲突文件，逐一 add 或 git add .
```
