...
本文介绍了一项新任务——视觉对话导航（Navigation from Dialog History），旨在训练agent在不确定情况下通过对话进行导航。研究基于MatterPort模拟器，收集了2050个人类导航对话，包含超过7k个导航轨迹。文中还定义了“从对话历史导航”任务，并通过实验证明对话历史对于理解上下文的重要性。
...
CVDN基于R2R任务，使用相同的模拟器和API训练导航agent。
...
对于对话数据集，通过Amazon Mechanical Turk收集人类对话，在每个人工智能任务中，工人阅读有关Navigator和Oracle的角色，并可以练习使用导航界面。一对工人通过聊天界面相互连接。
...
CVDN给agent提供了导航、提问和问答服务。
每个对话框都是一系列Navigator问题和Oracle答案交换，每个交换之后都有Navigator步骤。作者使用这种结构将对话框从对话框历史实例划分为导航。
