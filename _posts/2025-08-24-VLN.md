...
VLM主要是依赖于大量的image-text数据（high-quality videotext datasets），
...
4. 引入了一个数据集来提升VLN的泛化能力，VLN-CE-Isaac
...
而作者更是通过实验发现，所采用的VLA与运动控制（locomotion）的两级系统来实现VLN比起经典的VLN框架要提升17%的成功率。
