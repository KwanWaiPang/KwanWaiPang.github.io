...
这篇综述不仅仅是罗列论文，它横跨了机器人3D表征近50年的发展历程（从1970s到2025年），系统性地将表征方式划分为 **几何表征（Geometric）**、**神经表征（Neural）** 以及 **基础模型表征（Foundation Model）** 三大阶段，并深入探讨了它们在感知（Perception）、定位建图（Localization & Mapping）以及交互（Interaction，包括导航与操纵）中的应用。
