...
* 核心创新：它首次证明了纯前馈 Transformer 可以在完全不依赖相机内参（Calibration）或外参（Pose）的情况下，仅通过两张图像直接回归出 3D 点图（Pointmaps）。
* 技术深度：DUSt3R 将重建任务视为一个图像块（Patch）匹配与空间投影的联合学习过程。其输出不仅包括每张图的 3D 坐标，还包含一个置信度图（Confidence Map），用于在后续的全局对齐（Global Alignment）中过滤噪声。
* 局限性：DUSt3R 主要针对双视图（Pairwise），在处理多视图大规模场景时，其内存开销呈平方级增长，且缺乏显式的特征匹配引导。


**MASt3R（2024年）**是对 DUSt3R 的针对性升级，旨在解决多视图一致性与匹配精度问题。
* 核心创新：在 DUSt3R 的基础上引入了**特征对齐（Matching）**和 Sim(3) 几何约束。
* 技术深度：MASt3R 不再仅仅回归坐标，它同时学习具有几何意义的局部描述子。通过引入相似变换群（Sim(3)）的优化，它能够更稳定地处理缩放不确定性。这使得模型在极端视角变化下，其匹配能力远超传统的 SIFT 或 SuperPoint，为后续的大规模 SfM 提供了更坚实的底座。

**CUT3R（2025年）**解决了该系列模型在处理连续视频流时的“遗忘”与“计算冗余”问题。
* 核心创新：将原本静态的 Transformer 架构转变为递归式（stateful recurrent model）架构。
...
* 技术深度：
  * 内存优化：VGGT 通过在视图内注意力（Intra-view）和跨视图注意力（Inter-view）之间交替，将内存复杂度降至线性，这使得单次推理处理 200+ 张图像成为可能。
  * SL(4) 几何扎根：VGGT 显式地将单应性（Homography）等几何先验“植入”到 Transformer 的权重中，使其在推理速度（<1s）极快的同时，姿态估计精度（AUC@30）实现了质的飞跃。它不仅是目前最快的 SfM 方案，也是精度最高的方案之一。
