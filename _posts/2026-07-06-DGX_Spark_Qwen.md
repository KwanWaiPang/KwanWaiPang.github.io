...
| **统一内存**  | 128 GB（LPDDR5x, 273 GB/s 带宽） |
| **AI 算力**   | 1 PFLOP                          |
| **CUDA 版本** | 13.0                             |
| **网络**      | ConnectX + WiFi 7 + QSFP 40Gbps  |
...
接口布局如下图所示，注意**电源只能插在最左侧 Type-C 口（Power）**。QSFP 接口为四通道小型可插拔光模块接口，总传输速率可达 40Gbps。
...
3. **键盘布局和条款**：默认配置跳过
4. **用户创建**：设置用户名和密码
5. **WIFI 连接**：连接网络后，系统会自动下载并安装完整的软件镜像（此过程请勿中断）
6. **登录系统**

## 1.4 查看硬件信息

系统安装完成后，可以通过 `nvidia-smi` 查看显卡驱动和 GPU 信息（驱动已默认安装，CUDA 版本为 13.0，GPU 架构为 Blackwell SM121，配备 128 GB 统一内存）。

## 1.5 Conda 安装

下载 [Miniforge3-26.3.2-2-Linux-aarch64.sh](https://github.com/conda-forge/miniforge/releases/download/26.3.2-2/Miniforge3-26.3.2-2-Linux-aarch64.sh) 并执行安装：
...
本文完整记录了在 NVIDIA DGX Spark 上部署和加速 Qwen3.5-122B-A10B 的全过程。通过引入 FlashInfer 注意力后端、混合 INT4+FP8 量化、MTP-2 推测解码、以及 Triton INT8 LM Head v2 内核等优化手段，我们将模型在桌面超级计算机 DGX Spark 上的推理速度从 **15.44 tok/s 提升至 46.88 tok/s**，实现了近 **2.04 倍** 的性能增益。相比于 Marlin INT4 的优化基准（28.30 tok/s），也实现了 **+65.6%** 的显著加速。

整个优化方案的核心洞察是：DGX Spark 的 SM121 GPU 是一个**带宽受限**的平台（273 GB/s LPDDR5x），因此所有有效的优化都指向同一个方向——**减少内存读取量**。无论是 INT4/FP8 量化减少权重体积、INT8 LM Head 将 1.5GB 压缩为 729MB、还是 MTP 复用已有的轻量级预测头，本质上都是在有限带宽下榨取最大吞吐。
